...
public class Follow<T> : State<T>
{
    ...
    float RangeToFollow = 0;

    public Follow(List<Node> PathNode, Transform MyTransform, LayerMask Lay, float detection, Transform transformTarget, Vector3 Wapeon, float rangeToFollow)
    {
        path = PathNode;
        root = MyTransform;
        nodemask = Lay;
